<div>
    ...
    <div class="text-center">
        ...
        <h1 class="mt-4 text-xl font-semibold text-slate-900 tracking-tight">{{ __('Berlangganan NichmattPOS') }}</h1>
        ...
        @else
            <p class="mt-2 text-sm text-rose-600 font-medium">
    ...
    <div class="mt-6 rounded-2xl border border-slate-200 bg-slate-50/60 p-5">
        <div class="flex items-center justify-between">
            <div>
                <p class="font-medium text-slate-900">{{ __('Langganan Bulanan') }}</p>
                <p class="text-sm text-slate-500">{{ __('Akses penuh semua fitur, per toko') }}</p>
            </div>
            <p class="text-xl font-semibold text-brand-600">Rp {{ number_format(\App\Models\Store::SUBSCRIPTION_MONTHLY_PRICE, 0, ',', '.') }}</p>
        </div>
    </div>

    @if ($latestPayment && $latestPayment->status === 'pending')
        <p class="mt-4 rounded-lg bg-amber-50 border border-amber-100 px-4 py-2.5 text-sm text-amber-700 text-center">
    ...
    <div class="mt-6 flex flex-col items-center gap-3">
        ...
        <button wire:click="logout" class="text-sm text-slate-400 hover:text-slate-600">
            {{ __('Keluar') }}
        </button>
    </div>
</div>
