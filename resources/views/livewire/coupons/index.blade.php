<div class="space-y-6">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="relative w-full sm:w-72">
            ...
            <x-text-input wire:model.live.debounce.300ms="search" type="text" class="w-full pl-10" placeholder="Cari kupon..." />
        </div>
        <x-primary-button wire:click="createCoupon">
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="text-left text-xs font-medium uppercase tracking-wide text-slate-400 dark:text-slate-500 bg-slate-50/70 dark:bg-slate-800/40">
                        <x-th-sort field="code" label="{{ __('Kode') }}" :sortField="$sortField" :sortDirection="$sortDirection" />
                        <th class="px-6 py-3">{{ __('Nama') }}</th>
                        <th class="px-6 py-3">{{ __('Aturan') }}</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    @forelse ($coupons as $coupon)
                        <tr wire:key="coupon-{{ $coupon->id }}" class="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition">
                            <td class="px-6 py-3.5 text-slate-900 dark:text-slate-100 font-mono font-semibold">{{ $coupon->code }}</td>
                            <td class="px-6 py-3.5 text-slate-500 dark:text-slate-400">{{ $coupon->name }}</td>
                                ...
                                <div class="flex flex-wrap gap-1.5">
                                    ...
                                    @if ($coupon->hasGift())
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400">
                                            {{ __('+ Gratis :qty :name', ['qty' => $coupon->gift_qty, 'name' => $coupon->giftProduct?->name ?? '-']) }}
                                        </span>
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-3.5 text-slate-500 dark:text-slate-400 whitespace-nowrap">
                                @if ($coupon->starts_at || $coupon->ends_at)
                                    {{ $coupon->starts_at?->translatedFormat('d M Y') ?? '...' }} - {{ $coupon->ends_at?->translatedFormat('d M Y') ?? '...' }}
                                @else
                            ...
                            <td class="px-6 py-3.5 text-right whitespace-nowrap">
                                <div class="inline-flex items-center gap-1">
                                    <x-icon-button wire:click="editCoupon({{ $coupon->id }})" title="{{ __('Edit') }}" color="brand">
                                    ...
                                    <x-icon-button wire:click="delete({{ $coupon->id }})" wire:confirm="Hapus kupon ini?" title="{{ __('Hapus') }}" color="rose">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-slate-400 dark:text-slate-500">{{ __('Belum ada kupon.') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <div class="fixed inset-0 z-50 overflow-y-auto px-4 py-6">
        ...
        <div class="p-6">
            <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100">{{ $editingId ? __('Edit Kupon') : __('Tambah Kupon') }}</h3>

            <form wire:submit="save" class="mt-4 space-y-4">
                <div>
                    <x-input-label for="name" value="Nama Kupon" />
                    <x-text-input wire:model="name" id="name" type="text" class="block w-full" placeholder="mis. Promo Ulang Tahun" />
                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="code" value="Kode Kupon" />
                    <div class="mt-1 flex gap-2">
                        <x-text-input wire:model="code" id="code" type="text" class="block w-full uppercase tracking-widest" placeholder="mis. ULTAH25" />
                        <button type="button" wire:click="generateCode" class="shrink-0 px-3 py-2 text-xs font-medium text-brand-700 bg-brand-50 rounded-lg hover:bg-brand-100 dark:text-brand-300 dark:bg-brand-500/10 dark:hover:bg-brand-500/20">{{ __('Generate') }}</button>
                    </div>
                ...
                <label class="flex items-start gap-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-800/40 p-4 cursor-pointer">
                    <input type="checkbox" wire:model.live="includeDiscount" class="mt-1 rounded border-slate-300 text-brand-600 focus:ring-brand-500 dark:border-slate-600 dark:bg-slate-800">
                    <span class="block font-medium text-slate-900 dark:text-slate-100">{{ __('Sertakan diskon harga') }}</span>
                </label>

                @if ($includeDiscount)
                    <div class="pl-4 border-l-2 border-brand-200 dark:border-brand-800 space-y-3">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <x-input-label for="discountType" value="Tipe" />
                            ...
                            <div>
                                <x-input-label for="discountValue" value="Nilai Dasar" />
                                <x-text-input wire:model="discountValue" id="discountValue" type="number" min="0" class="block w-full" placeholder="0" />
                                <x-input-error :messages="$errors->get('discountValue')" class="mt-1" />
                            </div>
                        </div>

                        <label class="flex items-start gap-3 cursor-pointer">
                            <input type="checkbox" wire:model.live="isAgeBased" class="mt-1 rounded border-slate-300 text-brand-600 focus:ring-brand-500 dark:border-slate-600 dark:bg-slate-800">
                            <span>
                                <span class="block text-sm font-medium text-slate-900 dark:text-slate-100">{{ __('Nilai dinamis berdasar umur pelanggan') }}</span>
                                <span class="block text-xs text-slate-500 dark:text-slate-400">{{ __('Nilai akhir = Nilai Dasar + (umur pelanggan x pengali). Butuh customer dengan tanggal lahir dipilih saat dipakai di Kasir.') }}</span>
                            </span>
                        </label>

                        @if ($isAgeBased)
                            <div>
                                <x-input-label for="ageMultiplier" value="Pengali per Tahun Umur" />
                                <x-text-input wire:model="ageMultiplier" id="ageMultiplier" type="number" min="0" class="block w-full" placeholder="mis. 1 untuk diskon % = umur" />
                                <x-input-error :messages="$errors->get('ageMultiplier')" class="mt-1" />
                            </div>
                        @endif
                    </div>
                @endif
                ...
                <label class="flex items-start gap-3 rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-800/40 p-4 cursor-pointer">
                    <input type="checkbox" wire:model.live="includeGift" class="mt-1 rounded border-slate-300 text-brand-600 focus:ring-brand-500 dark:border-slate-600 dark:bg-slate-800">
                    <span class="block font-medium text-slate-900 dark:text-slate-100">{{ __('Sertakan hadiah produk gratis') }}</span>
                </label>
                ...
                <x-input-error :messages="$errors->get('includeDiscount')" class="mt-1" />
