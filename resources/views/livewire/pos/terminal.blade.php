<div x-data x-on:bill-ready.window="window.open('{{ route('pos.bill') }}', '_blank')" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-2 space-y-4">
        <div class="relative">
            <svg class="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-brand-500 dark:text-brand-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 6h.01M4 12h.01M4 18h.01M8 6h1m-1 6h1m-1 6h1m4-18v18m4-18v6m0 6v6" /></svg>
            <input
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-card border border-slate-200/70 dark:border-slate-800 p-5 flex flex-col h-fit lg:sticky lg:top-24">
        @if ($lastTransaction)
            <div class="text-center py-6 space-y-4">
                <span class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400">
                ...
                <div class="text-emerald-600 dark:text-emerald-400 font-semibold text-lg">{{ __('Transaksi Berhasil') }}</div>
                <div class="text-sm text-slate-500 dark:text-slate-400">{{ $lastTransaction->transaction_no }}</div>
                @if ($lastTransaction->customer_name)
                    <div class="text-sm text-slate-500 dark:text-slate-400">{{ __('Customer') }}: {{ $lastTransaction->customer_name }}</div>
                @endif
                <div class="text-2xl font-bold text-slate-900 dark:text-slate-100">Rp {{ number_format($lastTransaction->total, 0, ',', '.') }}</div>
                @if ($lastTransaction->payment_method === 'cash')
                    <div class="text-sm text-slate-500 dark:text-slate-400">{{ __('Kembalian') }}: Rp {{ number_format($lastTransaction->change_amount, 0, ',', '.') }}</div>
                @endif
        ...
        @else
            <div class="mb-4 rounded-xl border border-brand-100 dark:border-brand-500/20 bg-brand-50/60 dark:bg-brand-500/10 p-3">
                <x-input-label for="orderCodeInput" value="Input Kode Pesanan (Self Order)" class="text-brand-700 dark:text-brand-300" />
                <div class="mt-1.5 flex gap-2">
                    <input
                        wire:model="orderCodeInput"
                        wire:keydown.enter.prevent="claimCode"
                        id="orderCodeInput"
                        type="text"
                        maxlength="6"
                    ...
                    />
                    <x-secondary-button type="button" wire:click="claimCode" class="shrink-0">{{ __('Ambil') }}</x-secondary-button>
                </div>
            ...
            <div class="flex items-center justify-between mb-3">
                ...
                @if ($claimedSelfOrderId)
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-brand-50 dark:bg-brand-500/10 text-brand-700 dark:text-brand-300">{{ __('Dari Self Order') }}</span>
                @endif
            </div>
            ...
            <div class="mt-4 space-y-3 border-t border-slate-100 dark:border-slate-800 pt-4">
                <div class="flex justify-between text-sm">
                    ...
                    <span class="text-slate-900 dark:text-slate-100">Rp {{ number_format($this->subtotal, 0, ',', '.') }}</span>
                </div>
                ...
                <div>
                    ...
                    <select wire:model.live="paymentMethod" id="paymentMethod" class="block w-full mt-1 border-slate-200 bg-slate-50/60 focus:bg-white focus:border-brand-500 focus:ring-brand-500 rounded-lg shadow-sm text-slate-900 dark:border-slate-700 dark:bg-slate-800/60 dark:focus:bg-slate-800 dark:text-slate-100">
                ...
                @if ($paymentMethod === 'cash')
                    ...
                    <div class="flex justify-between text-sm">
                        ...
                        <span class="text-slate-900 dark:text-slate-100 font-medium">Rp {{ number_format($this->change, 0, ',', '.') }}</span>
                    </div>
                @endif
