<div class="space-y-6">
    <div class="bg-white dark:bg-slate-900 border border-slate-200/70 dark:border-slate-800 shadow-card rounded-2xl p-6">
        <h3 class="text-base font-semibold text-slate-900 dark:text-slate-100">{{ $editingId ? __('Edit Satuan') : __('Tambah Satuan') }}</h3>

        <form wire:submit="save" class="mt-4 flex flex-wrap items-start gap-3">
            <div class="flex-1 min-w-[200px]">
                <x-text-input wire:model="name" type="text" class="block w-full" placeholder="mis. kg, gram, liter, pcs" />
    ...
    <div class="relative w-full sm:w-72">
        ...
        <x-text-input wire:model.live.debounce.300ms="search" type="text" class="w-full pl-10" placeholder="Cari satuan..." />
    </div>
        ...
        <table class="min-w-full text-sm">
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                @forelse ($units as $unit)
                    <tr wire:key="unit-{{ $unit->id }}" class="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition">
                        <td class="px-6 py-3.5 text-slate-900 dark:text-slate-100 font-medium">{{ $unit->name }}</td>
                        <td class="px-6 py-3.5 text-right">
                            <div class="inline-flex items-center gap-1">
                                <x-icon-button wire:click="edit({{ $unit->id }})" title="{{ __('Edit') }}" color="brand">
                                ...
                                <x-icon-button wire:click="delete({{ $unit->id }})" wire:confirm="Hapus satuan ini?" title="{{ __('Hapus') }}" color="rose">
                ...
                @empty
                    <tr>
                        <td colspan="2" class="px-6 py-10 text-center text-slate-400 dark:text-slate-500">{{ __('Belum ada satuan.') }}</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
